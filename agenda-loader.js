(() => {
  ...
  let mesi=load(), mese=Math.max(0,Math.min(11,Number(localStorage.getItem(MONTH_KEY))||new Date().getMonth())), pagina=0, selected=null, mode="pencil", stroke=null, history=[],future=[];
  ...
  function boot(){
    ...
    const st=document.createElement("style");st.textContent=`
      .agenda-screen{position:fixed;inset:0;z-index:990;display:none;align-items:center;justify-content:center;padding:max(14px,env(safe-area-inset-top)) max(12px,env(safe-area-inset-right)) max(14px,env(safe-area-inset-bottom)) max(12px,env(safe-area-inset-left));background:linear-gradient(180deg,#eaf3fb,#f7f2fb)}.agenda-screen.aperto{display:flex}
      .agenda-stage{position:relative;width:min(96vw,1536px);aspect-ratio:1536/1024;max-height:94vh;touch-action:none;-webkit-touch-callout:none;user-select:none;-webkit-user-select:none}.agenda-bg{display:block;width:100%;height:100%;object-fit:contain;user-select:none;-webkit-user-drag:none}.agenda-layer{position:absolute;inset:0;overflow:hidden;pointer-events:none}.agenda-canvas{position:absolute;inset:0;width:100%;height:100%;pointer-events:auto;touch-action:none;-webkit-touch-callout:none;user-select:none;-webkit-user-select:none}
