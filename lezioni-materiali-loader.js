(() => {
  ...
  function start() {
    ...
    style.textContent = `
      ...
      .lm-notebook-tools{position:fixed;left:max(24px,env(safe-area-inset-left));bottom:max(24px,env(safe-area-inset-bottom));z-index:880;display:flex;align-items:center;gap:1px;padding:5px 6px 5px 22px;border:1.5px solid color-mix(in srgb,var(--lm-color) 68%,white);border-radius:18px;background:color-mix(in srgb,var(--lm-soft) 88%,white);box-shadow:0 7px 18px rgba(80,60,45,.13);touch-action:none;user-select:none;-webkit-user-select:none}.lm-tools-grip{position:absolute;left:5px;top:50%;transform:translateY(-50%);width:14px;height:27px;display:grid;place-items:center;color:color-mix(in srgb,var(--lm-dark) 62%,white);font-weight:900;font-size:14px;cursor:grab;touch-action:none}.lm-tools-grip:active{cursor:grabbing}.lm-notebook-tools button{appearance:none;border:0;width:36px;height:36px;border-radius:10px;background:transparent;color:var(--lm-dark);font-size:19px;font-weight:800;cursor:pointer;touch-action:manipulation}.lm-notebook-tools button.active{background:white;box-shadow:0 2px 8px rgba(45,50,60,.10)}.lm-notebook-tools button:disabled{opacity:.28}.lm-tool-eraser,.lm-tool-lasso{display:grid!important;place-items:center}.lm-tool-eraser svg,.lm-tool-lasso svg{width:21px;height:21px;display:block;overflow:visible}.lm-tool-eraser svg{width:22px;height:22px}.lm-tool-lasso svg{width:23px;height:23px}
      .lm-scroll-rail{position:fixed;right:max(7px,env(safe-area-inset-right));top:132px;bottom:92px;width:12px;z-index:870;display:flex;justify-content:center;touch-action:none;user-select:none;-webkit-user-select:none}.lm-scroll-track{position:relative;width:3px;height:100%;border-radius:999px;background:rgba(23,63,122,.10)}.lm-scroll-thumb{position:absolute;left:50%;top:0;transform:translateX(-50%);width:8px;min-height:48px;border-radius:999px;background:color-mix(in srgb,var(--lm-color) 78%,white);box-shadow:0 1px 5px rgba(45,55,70,.14);touch-action:none}.lm-scroll-rail.hidden{display:none}
      ...
      canvas.addEventListener("pointerdown",e=>{
        ...
        if(notebookMode==="eraser"){
          ...
          const eraseAt=ev=>{
            ...
            if(dateObj && objectContainsPoint(dateObj,p)) { page.objects=page.objects.filter(o=>o.id!==dateObj.id); page.date=""; }
            const next=[];
